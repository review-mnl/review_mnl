...
function isSuspendedAccountMessage(message) {
    ...
    return (
        (text.includes('suspend') || text.includes('disabled') || text.includes('blocked')) &&
        (text.includes('account') || text.includes('review center') || text.includes('administrator') || text.includes('admin'))
    );
}

function showSuspendedAccountScreen(message) {
    if (typeof document === 'undefined') return;
    if (document.getElementById('rmnlSuspendedScreen')) return;

    var style = document.getElementById('rmnlSuspendedStyle');
    if (!style) {
        style = document.createElement('style');
        style.id = 'rmnlSuspendedStyle';
        style.textContent = '' +
            '.rmnl-suspended-screen{position:fixed;inset:0;z-index:99999;display:flex;align-items:center;justify-content:center;padding:24px;' +
            'background:radial-gradient(circle at 85% 12%, #d8e7ff 0%, rgba(216,231,255,0.35) 32%, rgba(216,231,255,0) 55%),linear-gradient(135deg,#ecf3ff 0%,#e1ecff 45%,#d7e4ff 100%);}' +
            '.rmnl-suspended-card{width:min(620px,100%);background:#ffffff;border:1px solid #cfe0ff;border-radius:18px;padding:28px;box-shadow:0 24px 50px rgba(10,76,255,0.18);font-family:Poppins,sans-serif;}' +
            '.rmnl-suspended-badge{display:inline-block;padding:6px 12px;border-radius:999px;background:#eaf1ff;color:#0a4cff;font-size:12px;font-weight:700;letter-spacing:.04em;text-transform:uppercase;}' +
            '.rmnl-suspended-title{margin:12px 0 10px;color:#0a2a84;font-size:38px;line-height:1.08;font-weight:800;}' +
            '.rmnl-suspended-text{margin:0;color:#36508f;font-size:15px;line-height:1.6;}' +
            '.rmnl-suspended-actions{margin-top:22px;display:flex;gap:10px;flex-wrap:wrap;}' +
            '.rmnl-suspended-btn{border-radius:10px;padding:10px 16px;border:1px solid #bfd3ff;background:#ffffff;color:#1a3ea8;cursor:pointer;font-family:Poppins,sans-serif;font-size:14px;font-weight:600;}' +
            '.rmnl-suspended-btn.primary{background:#0a4cff;border-color:#0a4cff;color:#fff;}' +
            '.rmnl-suspended-btn:hover{filter:brightness(0.98);}' +
            '@media (max-width:640px){.rmnl-suspended-card{padding:20px}.rmnl-suspended-title{font-size:29px}}';
        document.head.appendChild(style);
    }

    var root = document.createElement('div');
    root.id = 'rmnlSuspendedScreen';
    root.className = 'rmnl-suspended-screen';
    root.innerHTML = '' +
        '<div class="rmnl-suspended-card" role="alertdialog" aria-labelledby="rmnlSuspTitle" aria-describedby="rmnlSuspText">' +
            '<span class="rmnl-suspended-badge">Account Disabled</span>' +
            '<h2 id="rmnlSuspTitle" class="rmnl-suspended-title">Your account has been suspended</h2>' +
            '<p id="rmnlSuspText" class="rmnl-suspended-text">' +
                'Your review center account is currently disabled by the administrator. You can still sign in, but system features are blocked while your account is suspended.' +
            '</p>' +
            '<div class="rmnl-suspended-actions">' +
                '<button type="button" class="rmnl-suspended-btn primary" id="rmnlContactSupportBtn">Contact Support</button>' +
                '<button type="button" class="rmnl-suspended-btn" id="rmnlLogoutBtn">Log out</button>' +
            '</div>' +
        '</div>';
    document.body.appendChild(root);

    var msgEl = root.querySelector('#rmnlSuspText');
    if (msgEl && message) {
        msgEl.textContent = message;
    }

    var contactBtn = root.querySelector('#rmnlContactSupportBtn');
    if (contactBtn) {
        contactBtn.addEventListener('click', function() {
            window.location.href = 'contact.html';
        });
    }
    var logoutBtn = root.querySelector('#rmnlLogoutBtn');
    if (logoutBtn) {
...
async function apiRequest(method, path, body, isFormData) {
    ...
    if (res.status === 403) {
        ...
        if (isSuspendedAccountMessage(errorMsg)) {
            showSuspendedAccountScreen(errorMsg);
        }
        throw new Error(errorMsg);
    }
...
const AdminAPI = {
    ...
    getActivityLogs: () =>
        apiRequest('GET', '/api/activity-logs'),

    createActivityLog: (payload) =>
        apiRequest('POST', '/api/activity-logs', payload),
};
